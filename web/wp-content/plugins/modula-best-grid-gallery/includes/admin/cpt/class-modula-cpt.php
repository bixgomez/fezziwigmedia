...
class Modula_CPT
{
	...
	public function save_meta_boxes($post_id, $post)
	{
		...
		if (isset($_POST['modula-images'])) {
			// Legacy metabox submits an empty hidden field; v2 editor persists via REST — never wipe images.
			$raw_images = wp_unslash($_POST['modula-images']); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			if (! is_string($raw_images) || '' === trim($raw_images)) {
				// Skip images meta write only (settings above may still have been saved).
			} else {
				$modula_images = $this->sanitize_images($raw_images);
