...
if ( !function_exists('foogallery_migrate_min_php_admin_notice' ) ) {
	/**
	 * Show an admin notice to administrators when the minimum PHP version could not be reached
	 */
	function foogallery_migrate_min_php_admin_notice() {
...
if ( !function_exists('foogallery_migrate_min_wp_admin_notice' ) ) {
	/**
	 * Show an admin notice to administrators when the minimum WP version could not be reached
	 */
	function foogallery_migrate_min_wp_admin_notice() {
        //only show the admin message to users who can install plugins
        if ( !current_user_can('install_plugins' ) ) { return; }
...
if ( version_compare( phpversion(), FOOGM_MIN_PHP, "<" ) ) {
    add_action( 'admin_notices', 'foogallery_migrate_min_php_admin_notice' );
	return false;
}

//check minimum WordPress version
global $wp_version;
if ( version_compare( $wp_version, FOOGM_MIN_WP, '<' ) ) {
    add_action( 'admin_notices', 'foogallery_migrate_min_wp_admin_notice' );
	return false;
}
